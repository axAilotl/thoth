"""CCF 0.1.2 conformance: new vectors and the section 13.8 regressions.

Covers the 0.1.2 delta against the vendored package:
...
- ``vectors/conformance-0.1.2.json`` — the twelve implementation-informed
...
def test_suppression_canonical_vector(ccf_vectors_dir, schemas):
    ...
    tokens = sorted(
        suppression.token_for_origin(
            key,
            source_id=preimage["source_id"],
            native_id=preimage["native_id"],
            revision=preimage["revision"],
            object_kind=preimage["object_kind"],
        )
        if preimage["kind"] == "origin"
        else suppression.token_for_content(
            key,
            content_class=preimage["content_class"],
    ...
    content = next(p for p in vector["preimages"] if p["kind"] == "content")
    raw_digest = bytes.fromhex(content["content_digest"].removeprefix("sha256:"))
    expected_content_token = "hmac-sha256:" + hmac.new(
        key,
        b"ccf:suppression-content:v1\0"
        + content["content_class"].encode("utf-8")
        + b"\0"
        + raw_digest,
        hashlib.sha256,
    ).hexdigest()
    assert expected_content_token in tokens

    assert suppression_set.scope_commitment(
        [{"object_id": object_id} for object_id in vector["scope_object_ids"]]
    ) == vector["expected_scope_commitment"]
    assert vector["record_structural_payload"]["scope_commitment"] == (
        vector["expected_scope_commitment"]
    )
    ...
    schemas.validate(
        "urn:ccf:schema:0.1.2:structural.lineage.erasure_receipt",
...
# ---------------------------------------------------------------------------
# vectors/admission-authority-classes.json (spec 5.5, 0.1.2 registry)
# ---------------------------------------------------------------------------
...
def test_authority_class_vector_matrix(ccf_package_root, ccf_vectors_dir):
    ...
    for case in vector["cases"]:
        ...
        if case["expected"] == "accept":
            ...
            # values, verbatim (0.1.2 pins them prose-only).
...
def _erase(rig, object_id, *, authorized_producers=()):
    ...
    request = svc.submit_request(
        ...
        reason="0.1.2 suppression drill",
...
def test_erasure_requires_suppression_key(rig):
    """0.1.2: the receipt schema requires the suppression commitment, so
...
# ---------------------------------------------------------------------------
# vectors/conformance-0.1.2.json: coverage gate (spec 13.8)
# ---------------------------------------------------------------------------
...
CONFORMANCE_CASE_TESTS = {
    ...
    "origin-same-kind": {
        "test_ccf_012.py": {"test_origin_same_kind_multiplicity"},
    },
...
def test_conformance_case_coverage(ccf_vectors_dir, load_ccf_json):
    """Every pinned 0.1.2 conformance case has named test coverage."""
    import ast

    vector = load_ccf_json(ccf_vectors_dir / "conformance-0.1.2.json")
