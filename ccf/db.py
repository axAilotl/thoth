...
CCF_MIGRATIONS: tuple[PostgresMigration, ...] = (
    ...
    PostgresMigration(
        version=3,
        name="0003_governance_baseline",
        statements=(
            # Governance generation fences (spec section 9.5). Governance
            # mutations bump the matching ``governance.*`` fence in the
            # admission transaction; cached decisions record the generation
            # vector they were computed against and are only served while
            # every generation matches. ``projection.*`` fences use the
            # separate ``generation_fence`` table from migration 0002.
            """
            CREATE TABLE IF NOT EXISTS governance_fence (
                archive_id              text NOT NULL REFERENCES archive(archive_id),
                fence                   text NOT NULL,
    ...
    # foreign-custody / fork evidence. Version 5: the parallel erasure
    # stream owns 0004; the slots integrate into a contiguous sequence.
    PostgresMigration(
        version=5,
...
def migrate_ccf_store(settings: CcfPostgresSettings) -> PostgresMigrationReport:
    ...
    with open_ccf_connection(settings) as conn:
        return apply_postgres_migrations(
            ...
            # Parallel work streams own disjoint migration slots (0004 is the
            # erasure stream's); the integrated branch is contiguous again.
            allow_gaps=True,
        )
