...
import { blobContentCommitment, compartmentCommitment, suppressionMerkleRoot, suppressionToken } from './ccf-jcs.mjs';

const ROOT = path.resolve(path.dirname(new URL(import.meta.url).pathname), '..');
const vector = JSON.parse(fs.readFileSync(path.join(ROOT, 'vectors', 'conformance-0.1.2-rc1.json')));
...
const derivedTokens=suppression.preimages.map((preimage)=>suppressionToken(Buffer.from(suppression.key_hex,'hex'),preimage)).sort();
...
console.log(`All ${passed.size} portable CCF 0.1.2-rc1 conformance cases pass.`);
