...
export function suppressionToken(key, canonicalPreimage) {
  ...
  const required = canonicalPreimage.kind === 'origin'
    ? ['format', 'kind', 'source_id', 'native_id', 'revision', 'object_kind']
    : canonicalPreimage.kind === 'content'
      ? ['format', 'kind', 'content_commitment']
      : null;
  if (required === null || canonicalPreimage.format !== 'ccf.suppression-preimage/1') throw new Error('unknown suppression preimage format or kind');
  ...
  if (canonicalPreimage.kind === 'origin') {
  ...
  } else if (!/^sha256:[0-9a-f]{64}$/.test(canonicalPreimage.content_commitment)) {
    throw new Error('invalid suppression content commitment');
  }
