...
# Registry validation.
registry_specs=[
 ('types.registry.json','urn:ccf:schema:0.1.2-rc1:registries.type-registry'),
 ('links.registry.json','urn:ccf:schema:0.1.2-rc1:registries.link-registry'),
 ('blobs.registry.json','urn:ccf:schema:0.1.2-rc1:registries.blob-registry'),
 ('predicates.registry.json','urn:ccf:schema:0.1.2-rc1:registries.predicate-registry'),
 ('state-machines.registry.json','urn:ccf:schema:0.1.2-rc1:registries.state-machine-registry'),
 ('profiles.registry.json','urn:ccf:schema:0.1.2-rc1:registries.profile-registry'),
 ('admission-authority-classes.registry.json','urn:ccf:schema:0.1.2-rc1:registries.admission-authority-class-registry'),
 ('suppression-profiles.registry.json','urn:ccf:schema:0.1.2-rc1:registries.suppression-profile-registry'),
]
loaded={}
...
for i, preimage in enumerate(suppression['preimages']):
    validate('urn:ccf:schema:0.1.2-rc1:security.suppression-preimage',preimage,f'suppression preimage {i}')
suppression_preimage_validator=Draft202012Validator(schemas['urn:ccf:schema:0.1.2-rc1:security.suppression-preimage'],registry=registry,format_checker=FormatChecker())
...
validate('urn:ccf:schema:0.1.2-rc1:structural.lineage.suppression_set',suppression['record_structural_payload'],'suppression Record payload')
validate('urn:ccf:schema:0.1.2-rc1:objects.blob-structural-content',suppression['blob_structural_content'],'suppression Blob structural content')
validate('urn:ccf:schema:0.1.2-rc1:objects.blob-semantic-content',suppression['blob_semantic_content'],'suppression Blob semantic content')
validate('urn:ccf:schema:0.1.2-rc1:structural.lineage.erasure_receipt',suppression['receipt_structural_payload'],'suppression erasure receipt payload')
...
for kind in ['record','link','blob']:
    for hp in sorted(ex.glob(f'{kind}-*.header.json')):
        ...
        validate(f'urn:ccf:schema:0.1.2-rc1:objects.{kind}-header',h,hp.name)
        sp=hp.with_name(hp.name.replace('.header.json','.structural.json'))
        s=json.loads(sp.read_text());validate(f'urn:ccf:schema:0.1.2-rc1:objects.{kind}-structural',s,sp.name)
        sem_p=hp.with_name(hp.name.replace('.header.json','.semantic.json'))
        sem=json.loads(sem_p.read_text()) if sem_p.exists() else None
        if sem is not None: validate(f'urn:ccf:schema:0.1.2-rc1:objects.{kind}-semantic',sem,sem_p.name)
        st=s['content']
        if kind=='record':
            ...
            elif e['semantic_schema_id']!='urn:ccf:schema:0.1.2-rc1:payload.core.empty':
            ...
            validate('urn:ccf:schema:0.1.2-rc1:objects.blob-structural-content',st,sp.name+' content')

batch=json.loads((ex/'producer-batch.json').read_text());validate('urn:ccf:schema:0.1.2-rc1:sync.producer-batch',batch,'producer-batch.json')
...
manifest=json.loads((ROOT/'examples/mindpack/manifest.json').read_text());validate('urn:ccf:schema:0.1.2-rc1:objects.mindpack-manifest',manifest,'mindpack manifest')
...
print('\nAll CCF 0.1.2-rc1 package examples and registries validate.')
