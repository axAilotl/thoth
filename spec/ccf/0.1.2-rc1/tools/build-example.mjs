...
function uuid4(label) {
  const b = crypto.createHash('sha256').update(`ccf-0.1.2-rc1:${label}`).digest().subarray(0, 16);
  b[6] = (b[6] & 0x0f) | 0x40;
  b[8] = (b[8] & 0x3f) | 0x80;
...
function compartment(kind, comp, n, content) {
  return { format: `ccf.${kind}-${comp}/0.1.2-rc1`, salt: salt(n), content };
}
function header(kind, id, structural, semantic = null) {
  const h = {
    spec: 'ccf/0.1.2-rc1', object_kind: kind, id, hash_profile: 'ccf-jcs-sha256-v2',
...
function linkStructural(type, fromId, toId, retention = 'structural_retention_required', payload = {}, visibility = 'clear') {
  ...
  const out = { type: visibility === 'sealed' ? 'sealed.link' : type, type_version: 1, type_visibility: visibility, schema_digest: schemaDigest.get('urn:ccf:schema:0.1.2-rc1:objects.link-semantic-content'), registry_entry_digest: entryDigest(e), retention_profile: retention, structural_payload: payload, extensions: {} };
...
addRecord(ids.runtime, 'core.runtime', 3, { person_id: ids.person, recorded_by: ids.runtime, recorded_at: '2026-08-11T21:40:00.000Z', policy_ref: policyRef, authority: { basis: 'runtime_import', asserted_by: ids.runtime, accepted_by: null }, payload: { kind: 'backend', name: 'Thoth CCF adapter', version: '0.1.2-rc1-example', instance_id: 'thoth-local', capabilities: ['capture','transcribe','extract','sync'], operator_id: ids.person, extensions: {} }, extensions: {} });
...
function makeCommit(id, seq, parentHash, members, time) {
  ...
  const structuralContent = recordStructural('integrity.commit', 'epoch_lifetime_required', {
    archive_id: ids.archive, epoch_id: ids.epoch, sequence: String(seq), parent_commit_hash: parentHash, batch_merkle_root: merkle, member_count: String(members.length), hash_profile: 'ccf-jcs-sha256-v2', signature_profile: 'ed25519-jcs-v1', signer_key_id: ids.archiveKey, signer_public_key: archivePub, semantic_catalog_root: catalog.root, active_profiles: ['ccf-core-0.1.2-rc1','ccf-local-sync-0.1.2-rc1','ccf-continuity-pack-0.1.2-rc1'], committed_at: time
  });
  const headerForSigning = { spec: 'ccf/0.1.2-rc1', object_kind: 'record', id, hash_profile: 'ccf-jcs-sha256-v2', semantic_commitment: null };
...
fs.writeFileSync(path.join(MP,'README.md'),'# Example CCF 0.1.2-rc1 mindpack\n\nSelf-contained restore example generated from `tools/build-example.mjs`.\n');
...
const manifest={format:'ccf.mindpack/0.1.2-rc1',mode:'restore',pack_id:ids.pack,archive_id:ids.archive,epoch_id:ids.epoch,created_at:'2026-08-11T21:42:22.000Z',genesis_commit_hash:genesis.commit_hash,head_commit_hash:commit2.commit_hash,head_sequence:'2',semantic_catalog_root:catalog.root,hash_profile:'ccf-jcs-sha256-v2',profiles:['ccf-core-0.1.2-rc1','ccf-local-sync-0.1.2-rc1','ccf-continuity-pack-0.1.2-rc1'],counts:{records:String(records.length),links:String(links.length),blobs:String(blobs.length),commits:'3'},streams,external_dependencies:[],withheld:[],erased:[],foreign_custody_proofs:[],compartment_availability:compartmentAvailability,extensions:{}};
