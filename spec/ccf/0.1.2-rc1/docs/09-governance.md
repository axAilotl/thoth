...
`registries/admission-authority-classes.registry.json` pins every type-registry
authority class to accepted actor/envelope kinds, bases, person-acceptance
behavior, consulted canonical state, failure reason, and evaluator involvement.
Unknown classes and bases fail closed; implementations do not reinterpret the
catalog-pinned `ccf-admission-authority-v1` mapping.
