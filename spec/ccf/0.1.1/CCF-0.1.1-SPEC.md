...
`ccf-core-0.1.1` permits plaintext compartments protected by the deployment's filesystem, database, or volume encryption. Core still defines commitments, availability state, erasure receipts, and honest security claims.
...
The Archive returns `queued`, `committed`, `partial`, `rejected`, or `conflict`.
...
A cryptographic hash-profile change creates a new epoch linked to the final head of the previous epoch. Historical objects retain historical hashes. 0.1.1 does not permit an unannounced in-place hash-profile switch.
...
Published 0.1.1 schemas and profile semantics do not change in place.
...
Two independently writable descendants from one head are explicit forks. Imports preserve both heads and do not invent a winner. Automatic merge of first-person or governance state is outside 0.1.1; a reviewed destination process may create new adjudication Records.
