...
check(manifest.format === 'ccf.mindpack/0.1.1', 'manifest format');
...
for (const header of allHeaders) {
  check(header.spec === 'ccf/0.1.1', `object spec: ${header.id}`);
...
for (const header of blobs) {
  ...
  if (structural.availability === 'available') {
...
console.log(`Example CCF 0.1.1 mindpack verified: ${checks} checks passed.`);
