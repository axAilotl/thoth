...
**Status:** interoperability release
**Date:** 2026-08-13
**Reference package:** `ccf-0.1.2`
...
This package defines **Continuity Core Format 0.1.2**.
...
`ccf-core-0.1.2` is mandatory. Other profiles compose without changing the three-object model:

| Profile | Status | Purpose |
|---|---|---|
| `ccf-core-0.1.2` | required | Objects, compartments, hashing, admission, journal, lineages, baseline governance, mindpack |
| `ccf-local-sync-0.1.2` | recommended | Offline signed producer batches and resumable delta synchronization |
...
```json
{
  "format": "ccf.record-semantic/0.1.2",
  "salt": "...",
  "content": {
...
A cryptographic hash-profile change creates a new epoch linked to the final head of the previous epoch. Historical objects retain historical hashes. 0.1.2 does not permit an unannounced in-place hash-profile switch.
...
`vectors/mindpack-manifest-tamper.json` covers forged low/high counts, removed
and unlisted streams, both availability-state directions, missing and fabricated
dependencies, genesis/head changes, custody completeness, transfer mode, and
duplicate or contradictory availability rows. The package-native verifier
reconstructs ground truth independently and rejects every mutation.
