...
reproduction_package="$reproduction_root/ccf-0.1.2"
...
cp -af "$ROOT" "$reproduction_package"
mkdir "$published_metadata"
cp -f "$ROOT/SHA256SUMS" "$published_metadata/SHA256SUMS"
cp -f "$ROOT/PACKAGE-INVENTORY.md" "$published_metadata/PACKAGE-INVENTORY.md"
...
cp -f "$published_metadata/SHA256SUMS" SHA256SUMS
sha256sum --check --quiet SHA256SUMS

echo "CCF 0.1.2 artifacts reproduce byte-for-byte in a clean copy."
